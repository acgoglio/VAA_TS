...
for tg_idx,tg in enumerate(input_tg):
    ...
    # Output file
    fig_name = workdir+'/'+tg+'_'+time_p+pp_name+'_seiches.png'
                ...
                for atype_idx,atype in enumerate(input_type):
                    ...
                    for res_idx,res in enumerate(input_res):
                           ...
                           try: 
                              ...
                              var_mod_seiches,resp_seiches = mod.fft_2bands(line4plot,low_bound=1./(highf_seiches_band[0]), high_bound=1./(highf_seiches_band[1]),low_bound_1=1/(lowf_seiches_band[0]), high_bound_1=1/(lowf_seiches_band[1]),alpha=0.4,invert='True')
                              line4plot=var_mod_seiches-np.nanmean(var_mod_seiches)
                           except:
                              print (' ')

                           if idx_line_plot == 0:
                              # OBS SEICHES FILTER and PLOT 
                              var_obs_seiches,resp_seiches = mod.fft_2bands(var_obs,low_bound=1./(highf_seiches_band[0]), high_bound=1./(highf_seiches_band[1]),low_bound_1=1/(lowf_seiches_band[0]), high_bound_1=1/(lowf_seiches_band[1]),alpha=0.4,invert='True')
                              var_obs=var_obs_seiches-np.nanmean(var_obs_seiches)
                           ...
                           print ('prova time ',time4plot[43:48])
