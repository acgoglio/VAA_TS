...
# Color
#colors = pl.cm.Purples_r(np.linspace(0.1,0.9,3)) #13
...
for tg_idx,tg in enumerate(input_tg):
    ...
    # Output file
    fig_name = workdir+'/'+tg+'_'+time_p+'_atm_new_2022.png' #'zoom.png'
...
#                           globals()['diff_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res]=np.squeeze(globals()['var_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res])-var_obs
                           ...
                           if atype == 'FC1':
                              # Save the time for obs
                              print ('TIME FOR OBS')
                              alltimes_mod_tmp = globals()['alltimes_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res][:-42] #[:-200]
...
#    else:
    ...
    leg = plt.legend(loc='lower left',ncol=1,  shadow=True, fancybox=True, fontsize=16)
