...
for tg_idx,tg in enumerate(input_tg):
    ...
    # HF OBS
    ax.plot(np.squeeze(globals()['alltimes_mod_Hfreq_'+tg+'_'+dat+'_'+easys+'_'+'FCall_20191110'+'_w'+res])[:-6],var_obs_Hfreq[6:(24*3*6)]*100,'o-',color='orange',label='10 min freq. OBS (12 November peak = '+str(obs_Hfreq_max)+' cm)',linewidth=3,zorder=1)
    ...
    # Add mean obs offset
    plt.axhline(obs_mean*100,color='red',linewidth=2,linestyle='dashed',label='Mean OBS ('+str(round(obs_mean*100))+' cm)',zorder=0)
    ...
    print ('Prova ret',np.squeeze(globals()['alltimes_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res])[-53],np.squeeze(globals()['alltimes_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res])[-49])
    ...
    plt.title('Sea Level observations and MedFS forecasts at '+tg,fontsize=18) #and diff wrt obs in '+tg,fontsize=18)
    plt.ylim(0,200)
    if time_p == 'allp' :
