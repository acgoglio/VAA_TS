...
# Color
colors = ['lime','darkgreen','lightblue','tab:blue'] 
...
for tg_idx,tg in enumerate(input_tg):
    ...
    for dat_idx,dat in enumerate(input_dat):
        ...
        if dat == 'obs':
           # input files name
           file_to_open = input_dir+'/'+tg+'_'+dat+'.csv_ok.csv' #+'.nc'
...
for tg_idx,tg in enumerate(input_tg):
            ...
            for easys_idx,easys in enumerate(input_sys):
                ...
                for res_idx,res in enumerate(input_res):
                    ...
                    for atype_idx,atype in enumerate(input_type):
                           ...
                           max2print=int(np.max(np.squeeze(globals()['var_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res])[68:72]*100))
                           ...
                           if easys == 'EAS5' and atype != 'AN' :
                              ax.plot(np.squeeze(globals()['alltimes_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res]),np.squeeze(globals()['var_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res])*100,'-',color=colors[idx_line_plot],label=easys+'_'+atype[0:2]+'_20191110_w'+res+' (diff wrt Obs = '+str(diff2print)+' cm)',linewidth=3)
                           elif easys == 'EAS6' and atype != 'AN' and res != '08' :
                                 ax.plot(np.squeeze(globals()['alltimes_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res]),np.squeeze(globals()['var_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res])*100,'-',color=colors[idx_line_plot],label=easys+'_'+atype[0:2]+'_20191110_w'+res+' (diff wrt Obs = '+str(diff2print)+' cm)',linewidth=3)
                           elif res != '08' :
                              try:
                                 ax.plot(np.squeeze(globals()['alltimes_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res]),np.squeeze(globals()['var_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res])*100,'-',color=colors[idx_line_plot],label=easys+'_'+atype+'_20191110_w'+res+' (diff wrt Obs = '+str(diff2print)+' cm)',linewidth=3)
                              except:
                                 print ('No')
    ...
    ax.plot(np.squeeze(globals()['alltimes_mod_Hfreq_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res])[145:-2],var_obs_Hfreq[145+2:]*100,'o-',color='orange',label='High Freq. OBS (max='+str(obs_Hfreq_max)+' cm)',linewidth=3,zorder=0)
    ...
    plt.title('Sea Level forecast (10 November 2019 bulletin) at '+tg,fontsize=18) #and diff wrt obs in '+tg,fontsize=18)
    plt.ylim(0,200)
    if time_p == 'allp' :
