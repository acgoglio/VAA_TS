...
# ---  Input archive ---
if flag_dir == 0:
   input_dir          = '/data/cmcc/ag15419/tmp_med_dev_old/Venezia_Acqua_Alta_2019/VAA_wind_new2/'
elif flag_dir == 1:
   input_dir          = '/data/cmcc/ag15419/tmp_med_dev_old/Venezia_Acqua_Alta_2019/VAA_wind_dir_extr_all/'
...
for tg_idx,tg in enumerate(input_tg):
    ...
    # Output file
    if flag_dir == 1:
       fig_name = workdir+'/'+tg+'_'+time_p+'_winddir_FC3.png' #'zoom.png'
       print ('Working on Wind DIR')
    elif flag_dir == 0:
       fig_name = workdir+'/'+tg+'_'+time_p+'_wind_FC3.png' #'zoom.png'
...
#    else:
    ...
    if flag_dir == 0 :
       ...
       ax.plot(alltimes_obs_frommod[:-2],var_obs[1:-2],'-o',color=obs_color,label='OBS '+' (max: '+str(round(max_val,1))+' m/s)',linewidth=3)
    elif flag_dir == 1 :
       alltimes_obs_frommod = np.arange(alltimes_mod_tmp[0],alltimes_mod_tmp[-1], timedelta(hours=1)).astype(datetime)
       ...
       ax.plot(alltimes_obs_frommod,var_obs_dir[:-3],'-',color=obs_color,label='OBS '+' (max: '+str(int(max_val))+' deg)',linewidth=3)
       # TMP
                    ...
                    for res_idx,res in enumerate(input_res):
                        ...
                        if glob.glob(file_to_open):
                           ...
                           if easys == 'EAS56' and atype != 'AN' and res == '10' and atype == 'FC3' :
                              ax.plot(np.squeeze(globals()['alltimes_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res]),np.squeeze(globals()['var_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res]),'-',color=colors[idx_line_plot],label='ECMWF '+atype+' (max: '+str(round(np.max(np.squeeze(globals()['var_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res][:])),1))+' m/s)',linewidth=3)
    ...
    if flag_dir == 0 :
       ...
       leg = plt.legend(loc='upper right',ncol=1,  shadow=True, fancybox=True, fontsize=16)
       ax.grid('on')
       plt.title('10m Wind Speed at '+tg,fontsize=18)
       ...
       ax.grid('on')
       plt.title('10m Wind Direction at '+tg,fontsize=18)
