...
for tg_idx,tg in enumerate(input_tg):
    ...
    # Output file
    fig_name = workdir+'/'+tg+'_'+time_p+pp_name+'_surge.png'
                ...
                for atype_idx,atype in enumerate(input_type):
                    ...
                    for res_idx,res in enumerate(input_res):
                           ...
                           try: 
                              ...
                              var_mod_surge,resp_all_res = mod.fft_2bands(line4plot,low_bound=1./(highf_all_band[0]), high_bound=1./(highf_all_band[1]),low_bound_1=1/(lowf_all_band[0]), high_bound_1=1/(lowf_all_band[1]),alpha=0.4,invert='False')
                              line4plot=var_mod_surge-np.nanmean(var_mod_surge)+np.nanmean(var_obs_surge)
                           except:
                              print (' ')

                           if idx_line_plot == 0:
                              # OBS SEICHES FILTER and PLOT 
                              var_obs_surge,resp_all_res = mod.fft_2bands(var_obs,low_bound=1./(highf_all_band[0]), high_bound=1./(highf_all_band[1]),low_bound_1=1/(lowf_all_band[0]), high_bound_1=1/(lowf_all_band[1]),alpha=0.4,invert='False')
                              var_obs=var_obs_surge
