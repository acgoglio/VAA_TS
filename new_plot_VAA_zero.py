...
time_an9,an9 = read_nc_series('ISMAR_TG_mod_EAS9_AN_w10_Oct.nc','sossheig','time_counter')
time_anbt9,anbt9 = read_nc_series('ISMAR_TG_mod_EAS9BT_AN_w10_Oct.nc','sossheig','time_counter')
...
ax.plot(time_an9, an9, color='olive', label=f'EAS9 MedFS analysis')
...
# Plot the avg values
ax.set_xlim(datetime(2019,10,1), datetime(2019,11,1))
for i, y in enumerate(y_values):
    y_val = float(np.squeeze(y))
    #ax.axhline(y_val, color=cmap(i), linewidth=2.5, label=y_labels[i]+' '+str(round(y_values[i],1))+' cm')
...
# Time series
#ax.plot(time_obs_hf, obs_hf, 'b--', linewidth=2, label=f'10 min freq. OBS (12 November peak = {int(peak_obs_hf)} cm)')
ax.plot(time_obs_hourly, obs_hourly, 'o-', color='navy', label=f'Hourly OBS - Avg: '+str(np.round(np.nanmean(obs_hourly),1))+' cm', markersize=4)
#ax.plot(time_tpxo, tpxo, 'k--', label='Tides TPXO9')
...
time_an9, an9 = filter_time(time_an9, an9, datetime(2019,10,1), datetime(2019,11,1))
ax.plot(time_an9, an9, color='tab:orange', linewidth=2.5, label=f'EAS9 MedFS analysis - Avg: '+str(np.round(np.nanmean(an9),1))+' cm')

time_anbt9, anbt9 = filter_time(time_anbt9, anbt9, datetime(2019,10,1), datetime(2019,11,1))
ax.plot(time_anbt9, anbt9, color='magenta', linewidth=2.5, label=f'EAS9-BT model - Avg: '+str(np.round(np.nanmean(anbt9),1))+' cm')

# Layout e legenda
ax.set_ylim(-100, 100)
ax.set_ylabel('Sea Level [cm]', fontsize=20)
ax.set_xlabel('Days of October 2019', fontsize=20)
ax.set_title('Sea Level observations, EAS9 analysis and EAS9-BT model at ISMAR_TG', fontsize=20)
...
# Save the plot
plt.tight_layout()
plt.savefig('sea_level_ISMAR_TG_obs_Oct19.png', dpi=300)
