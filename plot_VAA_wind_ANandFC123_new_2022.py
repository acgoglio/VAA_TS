...
# -- Workdir path -- 
workdir = '/work/oda/med_dev/Venezia_Acqua_Alta_2019/VAA_wind_ts_new_2022_2//'

# -- Period --
start_date = 20221120 #12 #09
end_date   = 20221122 #13 #17
...
# ---  Input archive ---
input_dir          = '/work/oda/med_dev/Venezia_Acqua_Alta_2019/VAA_wind_ts_new_2022_2//'
#
input_tg   = ['ISMAR_TG']
...
colors = pl.cm.Purples(np.linspace(0.5,0.9,3))
#############################
...
for tg_idx,tg in enumerate(input_tg):
    ...
    # Output file
    fig_name = workdir+'/'+tg+'_'+time_p+'_wind_2022.png' #'zoom.png'
...
#                           globals()['diff_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res]=np.squeeze(globals()['var_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res])-var_obs
                           ...
                           if atype == 'FC1':
                              # Save the time for obs
                              alltimes_mod_tmp = globals()['alltimes_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res][:-40] #[:-167]
...
#                           globals()['diff_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res]=np.squeeze(globals()['var_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res])-var_obs
                           ...
                           print ('NOT Found!')
print ('time len',len(alltimes_mod_tmp))
...
#    else:
    ...
    ax.plot(alltimes_obs_frommod,var_obs[:-1],'-',color='red',label='OBS '+' (max: '+str(round(max_val,1))+' m/s)',linewidth=3)
                ...
                for atype_idx,atype in enumerate(input_type):
                    ...
                    for res_idx,res in enumerate(input_res):
                        ...
                        if glob.glob(file_to_open):
                           ...
                           ax.plot(np.squeeze(globals()['alltimes_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res])[1:],np.squeeze(globals()['var_mod_'+tg+'_'+dat+'_'+easys+'_'+atype+'_w'+res])[:-1],linetype,color=colors[idx_line_plot],label=labels,linewidth=3)
    ...
    leg = plt.legend(loc='upper center',ncol=1,  shadow=True, fancybox=True, fontsize=18)
    ...
    if time_p == 'allp' :
       ...
       plt.xlabel ('November 2022',fontsize=18)
       ...
       plt.xlim([datetime(2022,11,22,0,0,0),datetime(2022,11,22,23,30,0)])
       plt.xlabel ('November 2022',fontsize=18)
       ax.xaxis.set_major_locator(mdates.DayLocator())
       ax.xaxis.set_minor_locator(mdates.HourLocator((6,12,18)))
       ax.xaxis.set_major_formatter(mdates.DateFormatter("\n%d"))
       ...
       plt.ylim(2.5,20)
       plt.xlim([datetime(2019,11,22,16,30,0),datetime(2019,11,22,23,30,0)])
       plt.xlabel ('22 November 2022',fontsize=18)
