...
# -- Workdir path -- 
workdir = '/work/oda/ag15419/tmp/Venezia_Acqua_Alta_HH/VAA_wind_ts_new3/'
...
# ---  Input archive ---
if flag_dir == 0:
   input_dir          = '/work/oda/med_dev/Venezia_Acqua_Alta_2019/VAA_wind_new2/'
elif flag_dir == 1:
...
#    else:
    ...
    if flag_dir == 0 :
       ...
       ax.plot(alltimes_obs_frommod[:-2],var_obs[1:-2],'-o',color='red',label='OBS '+' (max: '+str(round(max_val,1))+' m/s)',linewidth=3)
    elif flag_dir == 1 :
       alltimes_obs_frommod = np.arange(alltimes_mod_tmp[0],alltimes_mod_tmp[-1], timedelta(hours=1)).astype(datetime)
